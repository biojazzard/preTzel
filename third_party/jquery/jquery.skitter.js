...
(function($) {
	...
	$sk.fn.extend({
		...
		setup: function()
		{
			...
			console.log(this.settings.width_skitter + ' x ' + this.settings.height_skitter);
			...
			var addImageLink = function (link, src, animation_type, label, target) {
				...
				if (self.settings.thumbs) {
					...
					self.box_skitter.find('.info_slide').append(
						...
						'<img class="'+animation_type+'" src="'+src+'" '+dimension_thumb+' />' +
						'</span> '
					);
				}
				else {
				...
				$.ajax({
					...
					success: function(data) {
						...
						$.each(data, function (key, value) {
							++u;
							var link      = (value['link']) ? value['link'] : '#';
							var src       = 'media/' + value['filename'];
							var animation_type  = $(this).find('image').attr('class');
							var label      = (value['title']) ? value['title'] : '!!!';
							var target      = (value['target']) ? value['target'] : '_self';
							addImageLink(link, src, animation_type, label, target);
						});
					}
				});
		...
		resizeImage: function(img_clone) 
		{
			if (this.settings.fullscreen) {
				...
				img_clone.find('img').css({'display': 'block'});
			}
			return img_clone;
		}, 
